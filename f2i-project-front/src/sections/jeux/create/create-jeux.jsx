...
const CreateUser = () => {
  ...
  const handleJeuxChange = (e) => {
    ...
    const updatedValue = (name === 'nombreDeTicketTotal' || name === 'grandLots') ? Number(value) : value;
    setJeuxData({ ...jeuxData, [name]: updatedValue });
  };

  const handleDetailChange = (index, name, value) => {
    const updatedValue = (name === 'valeurPrix' || name === 'nombreTicket') ? Number(value) : value;
  const updatedJeuxDetails = jeuxData.jeuxDetails.map((detail, detailIndex) => {
      ...
      <form onSubmit={handleSubmit}>
        ...
        <Paper style={{ padding: '16px', margin: '16px 0' }}>
          <Grid container spacing={2}>
              ...
              <React.Fragment key={index}>
                <Grid item xs={12} sm={3}>
                  <TextField
                    ...
                    onChange={(e) => handleDetailChange(index, 'nomPrix', e.target.value)}
                    variant="outlined"
                  />
                </Grid>
                <Grid item xs={12} sm={3}>
                  <TextField
                    ...
                    onChange={(e) => handleDetailChange(index, 'typePrix', e.target.value)}
                    variant="outlined"
                  />
                </Grid>
                <Grid item xs={12} sm={3}>
                  <TextField
                    ...
                    onChange={(e) => handleDetailChange(index, 'valeurPrix', e.target.value)}
                    variant="outlined"
                  />
                </Grid>
                <Grid item xs={12} sm={3}>
                  <TextField
                    ...
                    onChange={(e) => handleDetailChange(index, 'nombreTicket', e.target.value)}
                    variant="outlined"
                  />
                </Grid>
            ...
            <Grid item xs={12}>
              <Button startIcon={<AddCircleIcon />} onClick={addJeuxDetail} color="inherit">
                Add Jeux Detail
              </Button>
            </Grid>
          </Grid>
